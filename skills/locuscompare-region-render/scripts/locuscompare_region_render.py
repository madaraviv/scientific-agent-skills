...
# Resolve sibling-skill imports. This skill composes eqtl-catalogue-region-
# fetch, gwas-catalog-region-fetch and ld-1000g-region-compute, installed as
# sibling skill directories under skills/ with their code under scripts/, so
# each sibling module is resolved at skills/<sibling-skill>/scripts/. A
# required sibling that is absent raises an ImportError naming the skill to
# install, never a bare ModuleNotFoundError traceback. ukb-ppp-region-fetch
# is optional: without it the module loads and a pQTL exposure raises
# Tier2NotAvailable instead.
import importlib.util

_SCRIPTS_DIR = Path(__file__).resolve().parent
_SKILLS_ROOT = _SCRIPTS_DIR.parents[1]
_REQUIRED_SIBLING_SKILLS: dict[str, str] = {
    "eqtl_catalogue_region_fetch": "eqtl-catalogue-region-fetch",
    "gwas_catalog_region_fetch": "gwas-catalog-region-fetch",
    "ld_1000g_region_compute": "ld-1000g-region-compute",
    "ondemand_client": "ld-1000g-region-compute",
}
_OPTIONAL_SIBLING_SKILLS: dict[str, str] = {
    "ukb_ppp_region_fetch": "ukb-ppp-region-fetch",
}
for _p in (
    _SCRIPTS_DIR,
    *(
        _SKILLS_ROOT / _skill / "scripts"
        for _skill in dict.fromkeys(
            [*_REQUIRED_SIBLING_SKILLS.values(), *_OPTIONAL_SIBLING_SKILLS.values()]
        )
    ),
):
...
def sibling_skill_import_error(module_name: str, skill_name: str) -> ImportError:
    """The error raised when a required sibling skill's module is not importable."""
    return ImportError(
        f"locuscompare-region-render needs the sibling skill {skill_name!r} "
        f"(Python module {module_name!r}), expected at "
        f"{_SKILLS_ROOT / skill_name / 'scripts'}. Install that skill beside "
        f"this one under {_SKILLS_ROOT}, or add its scripts/ directory to "
        f"sys.path or PYTHONPATH."
    )


for _module, _skill in _REQUIRED_SIBLING_SKILLS.items():
    if importlib.util.find_spec(_module) is None:
        raise sibling_skill_import_error(_module, _skill)
...
try:
    ...
    UKB_PPP_AVAILABLE = True
except ModuleNotFoundError as _e:
    if _e.name != "ukb_ppp_region_fetch":
        raise
    # ukb-ppp-region-fetch is not installed beside this skill. pQTL
    # exposures (`UKB_PPP_*` study ids, `source: ukb_ppp`) are unavailable
    # until it is; the eQTL Catalogue path is unaffected.
    UKB_PPP_AVAILABLE = False
    UKBPPPRegionResult = None
    UKBPPPClient = None

    class UKBPPPAccessError(Exception):
        """Stand-in so the pQTL except-clause stays valid; never raised here."""
