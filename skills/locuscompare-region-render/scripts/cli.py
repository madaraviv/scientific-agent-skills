...
def _run(*, config: dict, config_dir: Path, output: Path) -> int:
    ...
    try:
        ...
        if e.name != "ukb_ppp_region_fetch":
            raise
        # ukb-ppp-region-fetch is not installed beside this skill; a config
        # asking for source=ukb_ppp is refused below with a message.
        UKBPPPClient = None
    ...
    if exposure_prefetched:
        ...
        if exposure_source == "eqtl_catalogue" and not eqtl_dataset_id:
            ...
            return 2
        if exposure_source == "ukb_ppp" and UKBPPPClient is None:
            print(
                "exposure.fetch.source=ukb_ppp needs the sibling skill "
                "ukb-ppp-region-fetch installed beside this one under skills/; "
                "it is not, so pQTL exposures are unavailable here.",
                file=sys.stderr,
            )
            return 2
